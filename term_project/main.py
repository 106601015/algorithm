...
def show_training_curve(train_history):
    ...
    plt.savefig(datetime.datetime.now().strftime("%Y_%m_%d__%H_%M_%S"))
    #plt.show()

def main(operator='train'):
    print('-----main init-----')
    term_project_path = os.path.join('C:\\', 'Users', 'user', 'Desktop', 'algorithm', 'term_project')
    print('***************> term_project_path:', term_project_path)
    model_dict = {
    ...
    if operator == 'train':
        ...
        for file_name in os.listdir(term_project_path):
            if file_name.split('.')[-1] == 'h5':
                ...
                print('{} test loss:'.format(file_name), score[0])
                print('{} test acc:'.format(file_name), score[1])
    else:
