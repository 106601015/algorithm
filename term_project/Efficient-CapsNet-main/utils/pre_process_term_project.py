...
# constants
TERM_PROJECT_IMG_SIZE = 28
TERM_PROJECT_TRAIN_IMAGE_COUNT = 2450
PARALLEL_INPUT_CALLS = 16

# normalize dataset
#def pre_process(image, label):
#    return (image / 256)[...,None].astype('float32'), tf.keras.utils.to_categorical(label, num_classes=10)
# !!! you don't need to preprocess term_project npy data, it has been preprocessed !!!

def image_shift_rand(image, label):
    image = tf.reshape(image, [TERM_PROJECT_IMG_SIZE, TERM_PROJECT_IMG_SIZE])
    ...
    right_margin = TERM_PROJECT_IMG_SIZE - tf.reduce_max(nonzero_x_cols) - 1
    top_margin = tf.reduce_min(nonzero_y_rows)
    bot_margin = TERM_PROJECT_IMG_SIZE - tf.reduce_max(nonzero_y_rows) - 1
    ...
    image = tf.reshape(image, [TERM_PROJECT_IMG_SIZE * TERM_PROJECT_IMG_SIZE])
    image = tf.roll(image, y_amt * TERM_PROJECT_IMG_SIZE, axis=0)
    image = tf.reshape(image, [TERM_PROJECT_IMG_SIZE, TERM_PROJECT_IMG_SIZE])
    image = tf.transpose(image)
    image = tf.reshape(image, [TERM_PROJECT_IMG_SIZE * TERM_PROJECT_IMG_SIZE])
    image = tf.roll(image, x_amt * TERM_PROJECT_IMG_SIZE, axis=0)
    image = tf.reshape(image, [TERM_PROJECT_IMG_SIZE, TERM_PROJECT_IMG_SIZE])
    image = tf.transpose(image)
    image = tf.reshape(image, [TERM_PROJECT_IMG_SIZE, TERM_PROJECT_IMG_SIZE, 1])
    return image, label

def image_rotate_random_py_func(image, angle):
    rot_mat = cv2.getRotationMatrix2D(
        (TERM_PROJECT_IMG_SIZE/2, TERM_PROJECT_IMG_SIZE/2), int(angle), 1.0)
    rotated = cv2.warpAffine(image.numpy(), rot_mat,
        (TERM_PROJECT_IMG_SIZE, TERM_PROJECT_IMG_SIZE))
    return rotated
...
def image_erase_random(image, label):
    ...
    with sess.as_default():
        ...
        patch = tf.zeros([4, 4])
        mask = tf.pad(patch, [[x, TERM_PROJECT_IMG_SIZE-x-4],
            [y, TERM_PROJECT_IMG_SIZE-y-4]],
...
def image_squish_random(image, label):
    ...
    width_mod = tf.cast(tf.floor(
        (rand_amts[0] * (TERM_PROJECT_IMG_SIZE / 4)) + 1), tf.int32)
    offset_mod = tf.cast(tf.floor(rand_amts[1] * 2.0), tf.int32)
    offset = (width_mod // 2) + offset_mod
    image = tf.image.resize(image,
        [TERM_PROJECT_IMG_SIZE, TERM_PROJECT_IMG_SIZE - width_mod],
        method=tf2.image.ResizeMethod.LANCZOS3,
        preserve_aspect_ratio=False,
        antialias=True)
    image = tf.image.pad_to_bounding_box(
        image, 0, offset, TERM_PROJECT_IMG_SIZE, TERM_PROJECT_IMG_SIZE + offset_mod)
    image = tf.image.crop_to_bounding_box(
        image, 0, 0, TERM_PROJECT_IMG_SIZE, TERM_PROJECT_IMG_SIZE)
    return image, label
...
def generate_tf_data(X_train, y_train, X_test, y_test, batch_size):
	...
	dataset_train = dataset_train.shuffle(buffer_size=TERM_PROJECT_TRAIN_IMAGE_COUNT)
