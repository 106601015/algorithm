...
import os
from utils import pre_process_mnist, pre_process_multimnist, pre_process_smallnorb, pre_process_term_project
import json
...
class Dataset(object):
    ...
    def get_dataset(self):
        if self.model_name == 'MNIST':
            ...
            print(self.X_train.shape, self.y_train.shape, self.X_test.shape, self.y_test.shape, type(self.X_train.shape))
            ...
            self.class_names = list(range(10))
            print("[INFO] Dataset loaded!")
        elif self.model_name == 'TERM_PROJECT':
            term_project_path = './term_project'
            self.X_train, self.y_train = np.load(os.path.join(term_project_path, 'train_data_x.npy')), np.load(os.path.join(term_project_path, 'train_data_y.npy'))
            self.X_test, self.y_test = np.load(os.path.join(term_project_path, 'test_data_x.npy')), np.load(os.path.join(term_project_path, 'test_data_y.npy'))
            self.class_names = list(range(10))
            print(self.X_train.shape, self.y_train.shape, self.X_test.shape, self.y_test.shape, type(self.X_train.shape))
            print("[INFO] Dataset loaded!")
    ...
    def get_tf_data(self):
        print('----> get_tf_data')
        dataset_train, dataset_test = 0, 0
        if self.model_name == 'MNIST':
            print('----> get_tf_data MNIST')
            ...
            print('----> get_tf_data TERM_PROJECT')
            dataset_train, dataset_test = pre_process_term_project.generate_tf_data(self.X_train, self.y_train, self.X_test, self.y_test, self.config['batch_size'])
